        ...
        pkg update -y && pkg upgrade -y || { echo "خطا در به‌روزرسانی پکیج‌های Termux. لطفاً اتصال اینترنت خود را بررسی کنید."; exit 1; }
        ...
        pkg install xray -y || { echo "خطا در نصب Xray. لطفلاً مطمئن شوید Termux به درستی کار می‌کند."; exit 1; }
        ...
        # 4. از کاربر می‌خواهد آدرس مخزن گیت‌هاب پروژه اصلی را وارد کند
        echo ""
        read -p "لطفاً آدرس کامل مخزن گیت‌هاب پروژه اصلی (مثلاً https://github.com/yourusername/your-repo.git) را وارد کنید: " GITHUB_REPO_URL
